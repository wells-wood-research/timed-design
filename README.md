...
i. `{MODEL_NAME}.csv` , Contains probabilities for different amino acids at each position of the protein as predicted by the selected model.

ii. `{MODEL_NAME}.fasta` , Protein sequences generated by selecting the amino acid with the highest probability (ArgMax) at each position as indicated by the probability predictions in the `{MODEL_NAME}.csv` file.

iii. `{MODEL_NAME}.txt` , A file listing the the protein/ chain ID and the number of its residues for amino acid sequences found in {MODEL_NAME}.fasta file.  

iv. `dataset.fasta` , Fasta sequences fed into the model.

v. `datasetmap.txt` , Fasta sequences fed into the model.

vi. `encoded_labels.csv` , Sequences from `dataset.fasta` encoded in labels.  
...
### 2.1. Generating the input files for Monde Carlo sampling
...
This will sample 200 sequences using the provided probability distribution and temperature factor of 0.1. A higher temperature will lead to more diverse sequences being sampled with the temperature range being 0.1-5. Output sampled sequences can be increased or decreased by adjusting the number of output structures ''--sample_n 200''.
